...
public class TaskServiceTests
{
    ...
    private static TaskItem MakeTask(string title = "Test", bool completed = false, string priority = "medium", DateTime? dueDate = null) =>
        new() { Id = Guid.NewGuid(), Title = title, IsCompleted = completed, Priority = priority, DueDate = dueDate, CreatedAt = DateTime.UtcNow };
    ...
    public async Task GetTasksAsync_ReturnsAllTasks()
    {
        ...
        _repo.Setup(r => r.GetAllAsync()).ReturnsAsync(tasks);

        var result = await CreateService().GetTasksAsync("all");
    ...
    public async Task GetTasksAsync_ActiveFilter_ReturnsOnlyActiveTasks()
    {
        ...
        _repo.Setup(r => r.GetByStatusAsync(false)).ReturnsAsync(tasks.Where(t => !t.IsCompleted));

        var result = await CreateService().GetTasksAsync("active");
    ...
    public async Task GetTasksAsync_CompletedFilter_ReturnsOnlyCompletedTasks()
    {
        ...
        _repo.Setup(r => r.GetByStatusAsync(true)).ReturnsAsync(tasks);

        var result = await CreateService().GetTasksAsync("completed");
    ...
    public async Task GetTasksAsync_OverdueFilter_ReturnsOnlyOverdueTasks()
    {
        ...
        _repo.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<TaskItem> { overdue, future });

        var result = await CreateService().GetTasksAsync("overdue");
    ...
    {
        await Assert.ThrowsAsync<ValidationException>(() => CreateService().GetTasksAsync("unknown"));
    }
    ...
    public async Task GetTaskByIdAsync_ReturnsTask_WhenFound()
    {
        ...
        _repo.Setup(r => r.GetByIdAsync(task.Id)).ReturnsAsync(task);

        var result = await CreateService().GetTaskByIdAsync(task.Id);
    ...
    {
        _repo.Setup(r => r.GetByIdAsync(It.IsAny<Guid>())).ReturnsAsync((TaskItem?)null);

        await Assert.ThrowsAsync<NotFoundException>(() => CreateService().GetTaskByIdAsync(Guid.NewGuid()));
    }
    ...
    public async Task CreateTaskAsync_CreatesTaskWithCorrectFields()
    {
        ...
        var result = await CreateService().CreateTaskAsync(dto);
    ...
    public async Task UpdateTaskAsync_UpdatesAndReturnsTask()
    {
        ...
        var result = await CreateService().UpdateTaskAsync(existing.Id, dto);
    ...
    public async Task UpdateTaskAsync_ThrowsNotFoundException_WhenNotFound()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() => CreateService().UpdateTaskAsync(Guid.NewGuid(), dto));
    }
    ...
    public async Task DeleteTaskAsync_DeletesTask_WhenFound()
    {
        var task = MakeTask();
        _repo.Setup(r => r.GetByIdAsync(task.Id)).ReturnsAsync(task);
        _repo.Setup(r => r.DeleteAsync(task.Id)).Returns(Task.CompletedTask);

        await CreateService().DeleteTaskAsync(task.Id);

        _repo.Verify(r => r.DeleteAsync(task.Id), Times.Once);
    }
    ...
    public async Task DeleteTaskAsync_ThrowsNotFoundException_WhenNotFound()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() => CreateService().DeleteTaskAsync(Guid.NewGuid()));
    }
}
